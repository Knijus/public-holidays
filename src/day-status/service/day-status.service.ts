...
import { getMonthName, HolidayService } from 'src/holiday/service/holiday.service';
...
export class DayStatusService {
	...
	async getDayStatus(countryCode: string, date: Date): Promise<any> {
		...
		const holidaysByMonths = await this.holidayService.getCountryHoliday(countryCode, year);
		console.log(holidaysByMonths);
		const isHoliday = IfDayIsHoliday(countryCode, date, holidaysByMonths);
		...
		.get(`${process.env.ENRICO_SERVICE}/${process.env.RESPONSE_TYPE}/${process.env.ENRICO_VERSION}?action=${process.env.ACTION_IS_WORK_DAY}&date=${dmyDate}&country=${countryCode}`)
		.toPromise();
		...
		await this.daysRepository.update({id:`${countryCode}${year}`}, {month:holidaysByMonths});
		return dayStatus;
	}
}
...
function IfDayIsHoliday(countryCode, date, holidayArr) {
	let isHoliday = undefined;
	for (let month in holidayArr) {
